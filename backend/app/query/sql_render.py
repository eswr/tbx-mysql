...
def render_predicate(
    ...
    if pred.operator == "=":
        ...
        param_name = f"p{param_counter}"
        params[param_name] = pred.value
        sql = dialect.like_clause(col, pred.value, pred.case_insensitive)
        ...
        param_name = f"p{param_counter}"
        params[param_name] = pred.value
        sql = dialect.like_clause(col, pred.value, True)
        return sql, param_counter + 1
    
    elif pred.operator == "IN":
        placeholders = [dialect.param_name(f"p{param_counter + i}") for i in range(len(pred.value))]
        for i, val in enumerate(pred.value):
            params[f"p{param_counter + i}"] = val
        return f"{col} IN ({', '.join(placeholders)})", param_counter + len(pred.value)
    
    elif pred.operator == "NOT IN":
        placeholders = [dialect.param_name(f"p{param_counter + i}") for i in range(len(pred.value))]
        for i, val in enumerate(pred.value):
            params[f"p{param_counter + i}"] = val
        return f"{col} NOT IN ({', '.join(placeholders)})", param_counter + len(pred.value)
...
def render_sql(plan: LogicalPlan, dialect: Dialect) -> CompiledQuery:
    ...
    from_clause = f"{plan.primary_table} t"
    
    # Add standard joins if needed
    # We always have: t (transaction) -> a (account) -> b (bank)
    if any("a." in col or "b." in col for col in plan.select_columns) or \
       any("a." in p.column or "b." in p.column for p in plan.predicates):
        from_clause += " JOIN account a ON t.account_id = a.account_id"
        from_clause += " JOIN bank b ON a.bank_code = b.bank_code"
...
def validate_sql_select_only(sql: str, dialect_name: str = "mysql"):
    ...
    for node in parsed.walk():
        ...
        if node_type in (
            "Insert", "Update", "Delete", "Drop", "Create", "Command", "Copy", "Alter",
            "Exec", "Execute", "Show", "Use",
        ):
