...
def resolve_date_range(
    spec: DateRangeType, context_date: date | None = None, month: str | None = None, year: int | None = None
) -> DateRange:
    """
    ...
    if spec == DateRangeType.CALENDAR_MONTH:
        ...
        n = 1  # The current parser supports one completed month.
        ...
        raise ValueError("Custom ranges require explicit start and end dates")
        ...
        label = (
            f"{_month_name(last_monday.month)} {last_monday.day} - {_month_name(last_sunday.month)} {last_sunday.day}"
        )
        ...
        n = 7  # The current parser supports the canonical seven-day window.
...
def previous_period(original_period: DateRange) -> DateRange:
    ...
    if (
        original_period.start.day == 1
        and (original_period.start.month % 12) + 1 == original_period.end.month
        or (original_period.start.month == 12 and original_period.end.month == 1)
    ):
