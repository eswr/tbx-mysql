...
UNSUPPORTED_DOMAINS = [
    ...
    (r"(?:\b(vendors?|suppliers?|payouts?)\b.*\b(owe|paid|pay|payment|spend)\b|\b(paid|pay|payment|spend)\b.*\b(vendors?|suppliers?)\b)", "vendor payables"),
...
def _parse_transaction_query(question: str, reference_date: date) -> FinancialQuery | QueryRefusal | None:
    ...
    if re.search(r"\b(which month|what month|month had the highest|per month|by month)\b", q_lower):
        ...
        group_by = []
    elif re.search(r"\btransactions?\b", q_lower) and not re.search(r"\b(how much|how many|count|total|spend|spent|spending)\b", q_lower):
        intent = Intent.TRANSACTION_LIST
        metric = Metric.TRANSACTION_COUNT
        aggregation = Aggregation.NONE
        group_by = []
    ...
    if date_spec is None:
        ...
        if bank_code and q_lower.strip() == f"show me {next((alias for alias, code in BANK_ALIASES.items() if code == bank_code and alias in q_lower), '')} transactions":
