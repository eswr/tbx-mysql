...
def load_mysql(db_url: str, csv_dir: str, clear: bool = True):
    ...
    try:
        ...
        with open(os.path.join(csv_dir, "bank.csv")) as f:
            ...
            for row in reader:
                cursor.execute(
                    "INSERT INTO bank (bank_code, bank_name) VALUES (%s, %s)", (row["bank_code"], row["bank_name"])
                )
        conn.commit()
        ...
        txn_count = txn_row["cnt"] if txn_row else 0
...
def load_duckdb(db_path: str, csv_dir: str):
    ...
    try:
        ...
        logger.info("Loading transactions...")
        conn.execute(
            f"INSERT INTO transaction SELECT * FROM read_csv_auto('{os.path.join(csv_dir, 'transaction.csv')}')"
        )
...
def main():
    ...
    parser.add_argument(
        "--db-url", help="MySQL connection URL (e.g., mysql://user:pass@host/db). If omitted, loads to DuckDB."
    )
