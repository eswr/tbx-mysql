...
def query_scalar(cursor, sql: str) -> any:
...
def main():
    ...
    try:
        ...
        total_debit = query_scalar(cursor, "SELECT SUM(transaction_amount) FROM `transaction` WHERE transaction_type='debit';")
        ...
        total_credit = query_scalar(cursor, "SELECT SUM(transaction_amount) FROM `transaction` WHERE transaction_type='credit';")
        ...
        ref_count = query_scalar(
            cursor,
            "SELECT COUNT(*) FROM `transaction` WHERE transaction_reference_id='S5314253';"
        )
        ...
        orphan_accounts = query_scalar(
            ...
            "WHERE NOT EXISTS (SELECT 1 FROM bank b WHERE b.bank_code = a.bank_code);"
        )
